import type { PortfolioItem, Skill } from './types';

export const skillsData: Skill[] = [
  { name: 'Angular', level: 85 },
  { name: 'Python', level: 68 },
  { name: 'Blender', level: 92 },
  { name: 'FL Studio', level: 91 },
  { name: 'Video', level: 95 },
  { name: 'Godot', level: 70 },
  { name: 'Gdevelop', level: 95 },
];

export const portfolioItems: PortfolioItem[] = [
  // Websites
  {
    id: 'w1',
    title: 'Waiting for Heaven',
    description: 'A dark psx-horror-game about a busstop.',
    category: ['website', 'game'],
    imageUrl: '/images/wfh.png',
    imageHint: 'album art',
    url: 'https://zwiebelonkel.github.io/WaitingForHeavenWebsite/',
    tags: ['Godot','Blender','FLStudio','ElevenLabs','Steam','GSAP','Anime.js'],
  },
  {
    id: 'w8',
    title: 'Outside Between',
    description: 'A social Webgame about Cards and Luck.',
    category: ['website', 'game'],
    imageUrl: '/images/outside.png',
    imageHint: 'album art',
    url: 'https://outside---between.web.app/',
    tags: ['Angular','Blender', 'TursoDB', 'Render', 'Cloudinary'],
  },
  {
    id: 'w2',
    title: 'McLearn',
    description: 'An educational web app for custom learn cards.',
    category: ['website'],
    imageUrl: '/images/learn.png',
    imageHint: 'McLearn',
    url: 'https://mclearn-d113a.web.app/',
    tags: ['Angular', 'TursoDB', 'Render', 'Cloudinary'],
  },
  {
    id: 'w3',
    title: 'Mouse Milker',
    description: 'An idle Webgame about milking a mouse',
    category: ['website', 'game'],
    imageUrl: '/images/mouse.png',
    imageHint: 'Mouse Milker',
    url: 'https://mouse-milker.web.app/',
    tags: ['React', 'Next.js', 'Tailwind', 'ElevenLabs'],
  },
  {
    id: 'w4',
    title: 'CAR BRO',
    description: 'A website for identifiying cars. Just upload a picture and see all important stats about your car.',
    category: ['website'],
    imageUrl: '/images/carbro.png',
    imageHint: 'CAR BRO',
    url: 'https://carbroki.web.app/',
    tags: ['Angular', 'TursoDB', 'Render', 'Gemini', 'AI'],
  },
  {
    id: 'w5',
    title: 'Werkstatt',
    description: 'A website for my favorite mechanic',
    category: ['website'],
    imageUrl: '/images/werkstatt.png',
    imageHint: 'Werkstatt',
    url: 'https://antondemo.web.app/',
    tags: ['React', 'Tailwind', 'Render', 'Google API', 'Calender'],
  },
  {
    id: 'w6',
    title: 'Fisherman Companion',
    description: 'Companion-App for Last Fisherman',
    category: ['website'],
    imageUrl: '/images/companion.png',
    imageHint: 'Werkstatt',
    url: 'https://last-fisherman.web.app/',
    tags: ['React', 'Tailwind', 'Piskel', 'Last Fisherman','Steam'],
  },
  {
    id: 'w7',
    title: 'Salary Live',
    description: 'Money Flow App',
    category: ['website'],
    imageUrl: '/images/salary.png',
    imageHint: 'Money Flow',
    url: 'https://zwiebelonkel.github.io/SalaryLive/',
    tags: ['HTML', 'CSS', 'JS'],
  },
  // Games
  {
    id: 'g1',
    title: 'Darkest Cards',
    description: 'A dark fantasy/horror card game about collecting cards.',
    category: ['game'],
    imageUrl: '/images/darkestCards.png',
    imageHint: 'Darkest Cards',
    url: 'https://zwiebelonkel.itch.io/darkest-cards',
    tags: ['Gdevelop', 'Piskel', 'FL Studio', 'Graphic'],
  },
  {
    id: 'g2',
    title: 'GAS-24',
    description: 'A 3D psx-styled horror game inside a gasstation.',
    category: ['game', '3d'],
    imageUrl: '/images/gas24.png',
    imageHint: 'Gas24',
    url: 'https://zwiebelonkel.itch.io/gas-24',
    tags: ['Godot', 'Blender', 'FL Studio', 'Graphic'],
  },
  {
    id: 'g3',
    title: 'Driver',
    description: 'A short pixelated horror game about driving a car and saving gasoline.',
    category: ['game'],
    imageUrl: '/images/driver.png',
    imageHint: 'Driver',
    url: 'https://zwiebelonkel.itch.io/driver',
    tags: ['Gdevelop', 'Piskel', 'FL Studio', 'Graphic'],
  },
  {
    id: 'g4',
    title: 'Fighter',
    description: 'A retro-inspired local multiplayer fighting game.',
    category: ['game'],
    imageUrl: '/images/fighter.png',
    imageHint: 'Fighter',
    url: 'https://zwiebelonkel.itch.io/fighter',
    tags: ['Gdevelop', 'Piskel', 'FL Studio', 'Graphic'],
  },
  {
    id: 'g5',
    title: 'Ball',
    description: 'An arcade ball-rolling game. Burrito bison principle',
    category: ['game'],
    imageUrl: '/images/ball.jpeg',
    imageHint: 'Ball',
    url: 'https://zwiebelonkel.itch.io/ball',
    tags: ['Gdevelop', 'Piskel', 'FL Studio', 'Graphic'],
  },
  {
    id: 'g6',
    title: 'Pitfall',
    description: 'A ragdoller with tricky traps and jumps.',
    category: ['game'],
    imageUrl: '/images/pitfall.png',
    imageHint: 'Pitfall',
    url: 'https://zwiebelonkel.itch.io/pitfall',
    tags: ['Gdevelop', 'Piskel', 'FL Studio', 'Graphic'],
  },
  {
    id: 'g7',
    title: 'Last Fisherman',
    description: 'A 3D fishing simulator.',
    category: ['game'],
    imageUrl: '/images/fishing.png',
    imageHint: 'Last Fisherman',
    url: 'https://store.steampowered.com/app/4262030/Last_Fisherman/',
    tags: ['Godot', 'Blender', 'FL Studio','Steam','Last Fisherman'],
  },
  {
    id: 'g8',
    title: 'Pizza Dude',
    description: 'A horror-themed indie game about a pizzeria.',
    category: ['game'],
    imageUrl: '/images/pizza.png',
    imageHint: 'Pizza Dude',
    url: 'https://zwiebelonkel.itch.io/daddy',
    tags: ['Gdevelop', 'Piskel', 'FL Studio', 'Graphic'],
  },

  // Music
  {
    id: 'm1',
    title: 'NIS',
    description: 'A oriental dark drill track on Spotify.',
    category: ['music'],
    imageUrl: '/images/nis.jpg',
    imageHint: 'NIS',
    url: 'https://open.spotify.com/intl-de/track/2QsA3EukXQY3S4S0CRqwpn?si=312acf9d7fe24b65',
    audioUrl: '/audio/sinsDemo.aac',
    tags: ['FL Studio'],
  },
  {
    id: 'm2',
    title: 'THUNDER',
    description: 'A dark piano drill/grime track on Spotify.',
    category: ['music'],
    imageUrl: '/images/thunder.jpeg',
    imageHint: 'THUNDER',
    url: 'https://open.spotify.com/intl-de/album/440FVIc8toL7h3851JPwxT?si=qKGh52WoT6SiW9N2UWPpSQ',
    audioUrl: '/audio/thunderDemo.aac',
    tags: ['FL Studio'],
  },
  {
    id: 'm3',
    title: 'WANTED',
    description: 'An ambient trap track hosted on Spotify.',
    category: ['music'],
    imageUrl: '/images/wanted.jpg',
    imageHint: 'WANTED',
    url: 'https://open.spotify.com/intl-de/track/6xQRBSiPcwjAjBELq987dS?si=5b0b8b27189f40ff',
    audioUrl: '/audio/wantedDemo.aac',
    tags: ['FL Studio'],
  },

  // 3D Models
  {
    id: 'd1',
    title: 'Orange Glasses',
    description: 'Interactive 3D model on Sketchfab.',
    category: ['3d'],
    imageUrl: '/images/glasses.png',
    imageHint: 'Orange glasses',
    url: '/models/brille.glb',
    tags: ['Blender'],
  },
  {
    id: 'd2',
    title: 'Office Assets',
    description: 'Architectural 3D scene on Sketchfab.',
    category: ['3d'],
    imageUrl: '/images/buro.png',
    imageHint: 'Office Assets',
    url: '/models/office.glb',
    tags: ['Blender'],
  },
  {
    id: 'd3',
    title: 'Cigarettes',
    description: 'Low poly game asset on Sketchfab. Made for Gas24',
    category: ['3d'],
    imageUrl: '/images/redSmoke.png',
    imageHint: 'Cigarettes',
    url: '/models/cigs.glb',
    tags: ['Blender'],
  },
  {
    id: 'd4',
    title: 'Beatstick',
    description: 'A music controler with motion-control, sequencer and samlple management. Made as a university project.',
    category: ['3d'],
    imageUrl: '/images/BeatstickCover.png',
    imageHint: 'Beatstick',
    url: '/models/beatstick.glb',
    tags: ['Blender'],
  },
  {
    id: 'd5',
    title: 'Magazine',
    description: 'A business-magazine used for Gas24.',
    category: ['3d'],
    imageUrl: '/images/magazine.png',
    imageHint: 'Beatstick',
    url: '/models/literature.glb',
    tags: ['Blender'],
  },

  // Links
  {
    id: 'l1',
    title: 'Itch.io Profile',
    description: 'Game development projects by Zwiebelonkel.',
    category: ['link'],
    imageUrl: '/images/itch.jpg',
    imageHint: 'Itch.io',
    url: 'https://zwiebelonkel.itch.io/',
  },
  {
    id: 'l2',
    title: 'GitHub Profile',
    description: 'Open source contributions and code repositories.',
    category: ['link'],
    imageUrl: '/images/github.png',
    imageHint: 'GitHub',
    url: 'https://github.com/Zwiebelonkel',
  },
  {
    id: 'l3',
    title: 'Sketchfab Profile',
    description: '3D models and visualizations by lucamuller2004.',
    category: ['link'],
    imageUrl: '/images/sketch.jpeg',
    imageHint: 'Cigarettes',
    url: 'https://sketchfab.com/lucamuller2004',
  },
  {
    id: 'l4',
    title: 'Spotify Artist Account',
    description: 'See all tracks here',
    category: ['link'],
    imageUrl: '/images/spotify.png',
    imageHint: 'Spotify',
    url: 'https://open.spotify.com/intl-de/artist/7Muwo2davwO0Ht8bGQiBZS?si=nCsfXq7USKC_skcGvGz8Wg',
  },
  {
    id: 'l5',
    title: 'Google PlayStore Account',
    description: 'See all released android apps here',
    category: ['link'],
    imageUrl: '/images/playstore.jpg',
    imageHint: 'Playstore',
    url: 'https://play.google.com/store/apps/dev?id=7237929260257047382&hl=de',
  },
  {
    id: 'l6',
    title: 'LinkedIn Account',
    description: 'See LinkedIn profile here',
    category: ['link'],
    imageUrl: '/images/linkedIn.png',
    imageHint: 'LinkedIn',
    url: 'https://www.linkedin.com/in/luca-m%C3%BCller-a182ab323/',
  },
  {
    id: 'l7',
    title: 'Steam Dev-Account',
    description: 'See Steam-Developer profile here',
    category: ['link'],
    imageUrl: '/images/steam.jpg',
    imageHint: 'Steam',
    url: '  https://store.steampowered.com/search/?developer=ZwiebelGames',
  },
  {
    id: 'v1',
    title: 'Motor',
    description: 'A short film about a car getting to life',
    category: ['video'],
    imageUrl: '/images/motor.jpg',
    imageHint: 'Motor',
    url: 'https://youtu.be/pbVQ6tK6uyU',
    tags: ['Adobe'],
  },
  {
    id: 'v2',
    title: 'Dieb',
    description: 'A short film about a dramatic chase-sequence that ends with confusion',
    category: ['video'],
    imageUrl: '/images/dieb.png',
    imageHint: 'Dieb',
    url: 'https://youtu.be/A3iveKzEfV4',
    tags: ['Adobe'],
  },
  {
    id: 'v3',
    title: 'Tunnels',
    description: 'A short video for a dome-mapping project',
    category: ['video'],
    imageUrl: '/images/tunnels.png',
    imageHint: 'Tunnels',
    url: 'https://youtu.be/x5AXkivYhog',
    tags: ['Adobe', 'Blender'],
  },
  {
    id: 'v4',
    title: 'FOODWARS',
    description: 'A short re-interpretation about the old ad "The Front Line"',
    category: ['video'],
    imageUrl: '/images/foodwars.png',
    imageHint: 'FOODWARS',
    url: 'https://youtu.be/4Yg9T_xH1KQ',
    tags: ['Adobe'],
  },
];
